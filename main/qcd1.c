...
static void read_actions(FILE *fpar)
{
   ...
   hmc=set_hmc_parms(nact,iact,npf,nmu,mu,nlv,tau,0);
...
static void read_observables(FILE *fpar)
{
   ...
   if (my_rank==0)
   {
      if (append)
      {
         ...
         error_root(itmp!=(stdint_t)(noms==0),1,"read_observables [qcd1.c]",
...
static void read_infile(int argc,char *argv[])
{
   ...
   if (my_rank==0)
   {
      ...
      error_root(fpar==NULL,1,"read_infile [qcd1.c]",
...
static void check_old_log(int ic,int *nl,int *icnfg)
{
   ...
   error_root(fold==NULL,1,"check_old_log [qcd1.c]",
...
static void check_old_ms3dat(int nl)
{
   ...
   error_root(fdat==NULL,1,"check_old_ms3dat [qcd1.c]",
              "Unable to open data file");
   ...
   for (ic=0;;ic++)
   {
      ...
      if (ir==0)
      {
         error_root(ic==0,1,"check_old_ms3dat [qcd1.c]",
      ...
      {
         dnt=nt-pnt;
         error_root(dnt<1,1,"check_old_ms3dat [qcd1.c]",
                    "Incorrect trajectory separation");
      }
      else if (ic>1)
         error_root(nt!=(pnt+dnt),1,"check_old_ms3dat [qcd1.c]",
   ...
   error_root((nt!=nl)||((ic>1)&&(dnt!=dtr_ms)),1,
              "check_old_ms3dat [qcd1.c]","Last trajectory numbers "
...
static void init_ud(void)
{
   ...
   if (scnfg)
   {
      if (cnfg[strlen(cnfg)-1]!='*')
      {
         ...
         error(cnfg_type!=gauge(),1,"init_ud [qcd1.c]",
