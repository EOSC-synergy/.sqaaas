...
int main(int argc,char *argv[])
{
   ...
   if (my_rank==0)
   {
      ...
      read_line("qhat","%d",&qhat_dgn);
      read_line("su3csw","%lf",&su3csw_dgn);
      read_line("u1csw","%lf",&u1csw_dgn);
      read_line("mu","%lf",&mu_dgn);
      read_line("ninv","%d",&ninv_dgn);
      read_line("nmr","%d",&nmr_dgn);
      read_line("ncy","%d",&ncy_dgn);
      if (bc!=3)
         read_line("cF","%lf",&cF_dgn);
      if (bc==2)
         read_line("cF'","%lf",&cF_prime_dgn);
      else
         cF_prime_dgn=cF_dgn;
      read_dprms("theta",3,theta_dgn);
   ...
   MPI_Bcast(&qhat_dgn,1,MPI_INT,0,MPI_COMM_WORLD);
   MPI_Bcast(&su3csw_dgn,1,MPI_DOUBLE,0,MPI_COMM_WORLD);
   MPI_Bcast(&u1csw_dgn,1,MPI_DOUBLE,0,MPI_COMM_WORLD);
   MPI_Bcast(&cF_dgn,1,MPI_DOUBLE,0,MPI_COMM_WORLD);
   MPI_Bcast(&cF_prime_dgn,1,MPI_DOUBLE,0,MPI_COMM_WORLD);
   MPI_Bcast(&mu_dgn,1,MPI_DOUBLE,0,MPI_COMM_WORLD);
   MPI_Bcast(theta_dgn,3,MPI_DOUBLE,0,MPI_COMM_WORLD);
   ...
   set_dirac_parms9(qhat,m0,su3csw,u1csw,cF,cF_prime,
