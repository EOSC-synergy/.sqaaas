...
static double diff_rat1(double ra,double rb,double mu0,double p,ratfct_t *rf)
{
   int k;
   double r,x,d,dmax;
   ...
   for (k=0;k<1000;k++)
   {
      ...
      d=fabs(1.0-(*rf).A*pow(x*x+mu0*mu0,-p)*eval_rat1(rf,x));
...
int main(int argc,char *argv[])
{
   ...
   p=(1.0*rp.power[0])/rp.power[1];
   dmax=diff_rat1(rp.range[0],rp.range[1],rp.mu0,p,rf);
