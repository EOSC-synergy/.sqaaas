...
* factors to the file <run name>.plot.ms1.dat in the plots directory. The
...
static void read_file_head(FILE *fdat)
{
   ...
   error(ir!=1,1,"read_file_head [read.ms1.c]",
   ...
   nrw=(int)(istd[0]);
   error(nrw<1,1,"read_file_head [read.ms1.c]",
         "nrw is out of range");

   nfct=malloc(2*nrw*sizeof(*nfct));
   error(nfct==NULL,1,"read_file_head [read.ms1.c]",
   ...
   error(ir!=(1+2*nrw),1,"read_file_head [read.ms1.c]",
         "Incorrect read count");
   error(ie!=0,1,"read_file_head [read.ms1.c]",
...
static void alloc_data(void)
{
   ...
   error((ppp==NULL)||(pp==NULL)||(p==NULL),1,"alloc_data [read.ms1.c]",
         "Unable to allocate data arrays");
...
static int read_data(FILE *fdat)
{
   ...
   error(ir!=(1+2*n),1,"read_data [read.ms1.c]",
...
static void cnfg_range(FILE *fdat,int *fst,int *lst,int *stp)
{
   ...
   error(nc==0,1,"cnfg_range [read.ms1.c]","No data records on data file");
   error(ie!=0,1,"cnfg_range [read.ms1.c]","Non-contiguous configuration numbers");
}
...
static void select_cnfg_range(FILE *fdat)
{
   ...
   error((step%stp)!=0,1,"select_cnfg_range [read.ms1.c]",
   ...
   error((last<first)||(((last-first)%step)!=0)||(((first-fst)%stp)!=0),1,
         "select_cnfg_range [read.ms1.c]","Improper configuration range");
...
static void alloc_avrw(void)
{
   ...
   pp=malloc(2*n2*sizeof(*pp));
   p=malloc(2*n3*sizeof(*p));
   error((ppp==NULL)||(pp==NULL)||(p==NULL),1,"alloc_avrw [read.ms1.c]",
         "Unable to allocate data arrays");
   avrw=ppp;
   lnrw=ppp+n1;
...
static void read_file(char *fin)
{
   ...
   fdat=fopen(fin,"rb");
   error(fdat==NULL,1,"read_file [read.ms1.c]","Unable to open data file");
   ...
   fclose(fdat);
   error((ims!=nms)||(data.nc!=last),1,"read_file [read.ms1.c]",
         "Incorrect read count");
...
static void print_plot(char *fin)
{
   ...
   error(p==NULL,1,"print_plot [read.ms1.c]","Unexpected data file name");
   n=p-fin;
   ...
   error(n>=NAME_SIZE,1,"print_plot [read.ms1.c]","File name is too long");
   strncpy(base,p,n);
   base[n]='\0';

   error(name_size("plots/%s.plot.ms1.dat",base)>=NAME_SIZE,1,
         "print_plot [read.ms1.c]","File name is too long");
   sprintf(plt_file,"plots/%s.plot.ms1.dat",base);
   fout=fopen(plt_file,"w");
   error(fout==NULL,1,"print_plot [read.ms1.c]",
...
int main(int argc,char *argv[])
{
   ...
   error(argc!=2,1,"main [read.ms1.c]","Syntax: read.ms1 <filename>");
