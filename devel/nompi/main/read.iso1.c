...
* flag iac and the average SU(3) and U(1) plaquettes to the file
* <run name>.plot.iso1.dat in the plots directory. In addition, some information
* about the distribution of dH and the integrated autocorrelation time of
* the plaquettes are printed to stdout.
*
*******************************************************************************/
...
static int read_dat(int n,dat_t *ndat,FILE *fin)
{
   ...
   for (i=0;i<n;i++)
   {
      ...
      ir+=fread(dstd,sizeof(double),3,fin);
...
static void read_file(char *fin)
{
   ...
   fdat=fopen(fin,"rb");
   error(fdat==NULL,1,"read_file [read.iso1.c]","Unable to open data file");
   ...
   error(nms==0,1,"read_file [read.iso1.c]",
         "Empty data file");

   adat=amalloc(nms*sizeof(*adat),3);
   error(adat==NULL,1,"read_file [read.iso1.c]",
         "Unable to allocate data array");

   fseek(fdat,ipos,SEEK_SET);
   error(read_dat(nms,adat,fdat)!=nms,1,"read_file [read.iso1.c]",
...
static void select_range(void)
{
   ...
   error(neff<2,1,"select_range [read.iso1.c]",
   ...
   error(ie!=0,1,"select_range [read.iso1.c]",
...
static void print_plot(char *fin)
{
   ...
   p=strstr(fin,".dat");
   error(p==NULL,1,"print_plot [read.iso1.c]","Unexpected data file name");
   n=p-fin;
   ...
   error(n>=NAME_SIZE,1,"print_plot [read.iso1.c]","File name is too long");
   strncpy(base,p,n);
   base[n]='\0';

   error(name_size("plots/%s.plot.iso1.dat",base)>=NAME_SIZE,1,
         "print_plot [read.iso1.c]","File name is too long");
   sprintf(plt_file,"plots/%s.plot.iso1.dat",base);
   fout=fopen(plt_file,"w");
   error(fout==NULL,1,"print_plot [read.iso1.c]",
   ...
   fprintf(fout,"#\n");
   fprintf(fout,"#  nt         dH      iac    <tr{U(p)}>    <u(p)>\n");
   fprintf(fout,"#\n");
   ...
   for (ims=0;ims<nms;ims++)
   {
      ...
      fprintf(fout," %.8e ",(*ndat).avpl3);
      fprintf(fout," %.8e",(*ndat).avpl1);
      fprintf(fout,"\n");
...
int main(int argc,char *argv[])
{
   ...
   error(argc!=2,1,"main [read.iso1.c]","Syntax: read.iso1 <filename>");
   ...
   error(a==NULL,1,"main [read.iso1.c]",
         "Unable to allocate data array");
   ...
   printf("********** SU(3) average plaquette\n\n");

   for (n=0;n<neff;n++)
      a[n]=adat[nfirst+n].avpl3;
   ...
   printf("********** U(1) average plaquette\n\n");

   for (n=0;n<neff;n++)
      a[n]=adat[nfirst+n].avpl1;
