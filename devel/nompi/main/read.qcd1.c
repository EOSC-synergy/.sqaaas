...
* flag iac and the average plaquette to the file <run name>.plot.qcd1.dat in the
...
static void read_file(char *fin)
{
   ...
   fdat=fopen(fin,"rb");
   error(fdat==NULL,1,"read_file [read.qcd1.c]","Unable to open data file");
   ...
   error(nms==0,1,"read_file [read.qcd1.c]",
         "Empty data file");

   adat=amalloc(nms*sizeof(*adat),3);
   error(adat==NULL,1,"read_file [read.qcd1.c]",
         "Unable to allocate data array");

   fseek(fdat,ipos,SEEK_SET);
   error(read_dat(nms,adat,fdat)!=nms,1,"read_file [read.qcd1.c]",
...
static void select_range(void)
{
   ...
   error(neff<2,1,"select_range [read.qcd1.c]",
   ...
   error(ie!=0,1,"select_range [read.qcd1.c]",
...
static void print_plot(char *fin)
{
   ...
   p=strstr(fin,".dat");
   error(p==NULL,1,"print_plot [read.qcd1.c]","Unexpected data file name");
   n=p-fin;
   ...
   error(n>=NAME_SIZE,1,"print_plot [read.qcd1.c]","File name is too long");
   strncpy(base,p,n);
   base[n]='\0';

   error(name_size("plots/%s.plot.qcd1.dat",base)>=NAME_SIZE,1,
         "print_plot [read.qcd1.c]","File name is too long");
   sprintf(plt_file,"plots/%s.plot.qcd1.dat",base);
   fout=fopen(plt_file,"w");
   error(fout==NULL,1,"print_plot [read.qcd1.c]",
...
int main(int argc,char *argv[])
{
   ...
   error(argc!=2,1,"main [read.qcd1.c]","Syntax: read.qcd1 <filename>");
   ...
   error(a==NULL,1,"main [read.qcd1.c]",
         "Unable to allocate data array");
