...
* 
* Based on openQCD-1.6/devel/uflds/check5.c
* Copyright (C) 2012, 2013, 2016 Martin Luescher
*
* This software is distributed under the terms of the GNU General Public
* License (GPL)
*
* Check of the program set_u1_bstap().
*
*******************************************************************************/
...
static double plaq0(int n,int ix)
{
   ...
   u1xu1(udb+ip[0],udb+ip[1],&wd1);
   u1dagxu1dag(udb+ip[3],udb+ip[2],&wd2);
...
int main(int argc,char *argv[])
{
   ...
   if (my_rank==0)
   {
      ...
      printf("\n");
      printf("Check of the program set_u1_bstap()\n");
      printf("----------------------------------\n\n");
      ...
      if (bc!=0)
         error_root(sscanf(argv[bc+1],"%d",&bc)!=1,1,"main [check7.c]",
                    "Syntax: check7 [-bc <type>] [-cs <cstar>]");

      cs=find_opt(argc,argv,"-cs");

      if (cs!=0)
         error_root(sscanf(argv[cs+1],"%d",&cs)!=1,1,"main [check7.c]",
                    "Syntax: check7 [-bc <type>] [-cs <cstar>]");
   }
   ...
   ie=check_bc(0.0);
   error_root(ie==0,1,"main [check7.c]","Boundary conditions changed");
