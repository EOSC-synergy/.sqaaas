...
static double su3dSdt(double mu0,double mu1,int ipf,int isp,int *status)
{
   mdflds_t *mdfs;
...
static double u1dSdt(double mu0,double mu1,int ipf,int isp,int *status)
{
   mdflds_t *mdfs;
...
int main(int argc,char *argv[])
{
   ...
   set_dfl_gen_parms(kappa,mu,-3,0.95,0.8,1.301,0.789,0.38,-1.25,0.54,ninv,nmr,ncy);
   ...
   for (isp=0;isp<3;isp++)
   {
      ...
      dsdt=su3dSdt(mu0,mu1,0,isp,status);
   ...
   for (isp=0;isp<3;isp++)
   {
      ...
      dsdt=u1dSdt(mu0,mu1,0,isp,status);
