...
static double su3dSdt(int *irat,int ipf,int isw,int isp,int *status)
{
   mdflds_t *mdfs;
...
static double u1dSdt(int *irat,int ipf,int isw,int isp,int *status)
{
   mdflds_t *mdfs;
...
{
   int my_rank,bc,cs,irat[3],isw,isp,status[6],mnkv;
   int Ns,nkv;
   int isap,idfl;
   ...
   if (my_rank==0)
   {
      ...
      if (bc!=0)
         error_root(sscanf(argv[bc+1],"%d",&bc)!=1,1,"main [check9.c]",
                    "Syntax: check9 [-bc <type>] [-cs <cstar>]");
   ...
   for (isw=0;isw<2;isw++)
   {
      for (isp=0;isp<3;isp++)
      {
         ...
         rot_ud(eps);
         dsdt=su3dSdt(irat,0,isw,isp,status);
   ...
   for (isw=0;isw<2;isw++)
   {
      for (isp=0;isp<3;isp++)
      {
         ...
         rot_ad(eps);
         dsdt=u1dSdt(irat,0,isw,isp,status);
