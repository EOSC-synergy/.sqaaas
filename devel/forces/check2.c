...
* License (GPL)
*
* SU(3) gauge action of constant Abelian background fields.
*
*******************************************************************************/
...
static double afld(int *x,int mu)
{
   ...
   xt[0]=(double)(safe_mod(x[0],np[0]));
   xt[1]=(double)(safe_mod(x[1],np[1]));
   xt[2]=(double)(safe_mod(x[2],np[2]));
   xt[3]=(double)(safe_mod(x[3],np[3]));
         ...
         for (x2=0;x2<L2;x2++)
         {
            for (x3=0;x3<L3;x3++)
            {
               ...
               for (k=1;k<4;k++)
               {
                  ...
                  rs1-=bf3*(cos(2.0*(phi[0]*inp[k]+r0))+
                            cos(2.0*(phi[1]*inp[k]+r0))+
                            cos(2.0*(phi[2]*inp[k]-2.0*r0))-3.0);

                  rs1-=bf2*(cos(2.0*phi[0]*inp[k]+r1)+
...
static double Amt(void)
{
   ...
   cs=1.0;
   if (bc_cstar()!=0) cs=0.5;
   
   return cs*(lat.beta/3.0)*(c0*(s0+bs0)+c1*(s1+bs1));
}
   ...
   if (my_rank==0)
   {
      ...
      if (bc!=0)
         error_root(sscanf(argv[bc+1],"%d",&bc)!=1,1,"main [check2.c]",
                    "Syntax: check2 [-bc <type>] [-sf <sf_type>] [-cs <cstar>]");

      sf=find_opt(argc,argv,"-sf");

      if (sf!=0)
         error_root(sscanf(argv[sf+1],"%d",&sf)!=1,1,"main [check2.c]",
                    "Syntax: check2 [-bc <type>] [-sf <sf_type>] [-cs <cstar>]");
