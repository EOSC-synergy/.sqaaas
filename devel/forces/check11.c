...
static double su3dSdt(double mu0,double mu1,int ipf,int isw,int isp,int *status)
{
   mdflds_t *mdfs;
...
static double u1dSdt(double mu0,double mu1,int ipf,int isw,int isp,int *status)
{
   mdflds_t *mdfs;
...
int main(int argc,char *argv[])
{
   ...
   if (my_rank==0)
   {
      ...
      if (bc!=0)
         error_root(sscanf(argv[bc+1],"%d",&bc)!=1,1,"main [check11.c]",
                    "Syntax: check11 [-bc <type>] [-cs <cstar>]");

      cs=find_opt(argc,argv,"-cs");

      if (cs!=0)
         error_root(sscanf(argv[cs+1],"%d",&cs)!=1,1,"main [check11.c]",
                    "Syntax: check11 [-bc <type>] [-cs <cstar>]");
   }
   ...
   for (isw=0;isw<2;isw++)
   {
      for (isp=0;isp<3;isp++)
      {
         ...
         rot_ud(eps);
         dsdt=su3dSdt(mu0,mu1,0,isw,isp,status);
   ...
   for (isw=0;isw<2;isw++)
   {
      for (isp=0;isp<3;isp++)
      {
         ...
         rot_ad(eps);
         dsdt=u1dSdt(mu0,mu1,0,isw,isp,status);
