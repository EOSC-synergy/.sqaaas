         ...
         for (x2=1;x2<(L2-2);x2++)
         {
            for (x3=1;x3<(L3-2);x3++)
            {
               ...
               for (mu=0;mu<4;mu++)
               {
                  ...
                  if (ix<(VOLUME/2))
                     frc=(*mdfs).su3frc+8*(iy-(VOLUME/2))+2*mu+1;
                  else
                     frc=(*mdfs).su3frc+8*(ix-(VOLUME/2))+2*mu;
...
int main(int argc,char *argv[])
{
   ...
   if (my_rank==0)
      printf("Comparison of fwd_su3_euler() and fwd_su3_rk2(): |dU| = %.1e\n",
             dev0);
   ...
   if (my_rank==0)
   {
      printf("Comparison of fwd_su3_rk2() and fwd_su3_rk3():   |dU| = %.1e\n\n",
             dev0);
      fclose(flog);
   }
