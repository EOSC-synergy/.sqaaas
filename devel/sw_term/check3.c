...
static int bc,np[4];
static double t[4],a[4],p[4],inp[4];
static double (*Fhat)[4];
...
static void set_parms(void)
{
   ...
   MPI_Bcast(t,4,MPI_DOUBLE,0,MPI_COMM_WORLD);
         ...
         for (x2=0;x2<L2;x2++)
         {
            for (x3=0;x3<L3;x3++)
            {
               ...
               if (((x[0]>0)&&((x[0]<(N0-1))||(bc!=0)))||(bc==3))
               {
                  ...
                  Fhat[ix][3]=0.25*(ftp[0][3]+ftp[1][3]+ftp[2][3]+ftp[3][3]);
               }
               else
               {
                  Fhat[ix][0]=0.0;
...
static void muladd_pauli(double u1csw,double csw,int mu,int nu,
                         spinor_dble *pk,spinor_dble *pl)
{
   ...
   for (ix=0;ix<VOLUME;ix++)
   {
      ...
      r=u1csw*Fhat[ix][3]+csw*Fhat[ix][0];
      ws.c1.c1.re*=r;
      ws.c1.c1.im*=r;
      ws.c2.c1.re*=r;
      ws.c2.c1.im*=r;
      ...
      r=u1csw*Fhat[ix][3]+csw*Fhat[ix][1];
      ws.c1.c2.re*=r;
      ws.c1.c2.im*=r;
      ws.c2.c2.re*=r;
      ws.c2.c2.im*=r;
      ...
      r=u1csw*Fhat[ix][3]+csw*Fhat[ix][2];
      ws.c1.c3.re*=r;
      ws.c1.c3.im*=r;
      ws.c2.c3.re*=r;
      ws.c2.c3.im*=r;
...
static void mul_swd(double m0,double u1csw,double csw,spinor_dble *pk,spinor_dble *pl)
{
   int mu,nu;
   double c;
   spinor_dble *pm;
   ...
   {
      for (nu=(mu+1);nu<4;nu++)
         muladd_pauli(2.0*u1csw,2.0*csw,mu,nu,pk,pl);
   }
...
int main(int argc,char *argv[])
{
   ...
   dp=set_dirac_parms9(q,-0.0123,csw[0],csw[1],cF[0],cF[1],theta[0],theta[1],theta[2]);
   print_dirac_parms();
   ...
   for (n=0;n<4;n++)
   {
      ...
      if (my_rank==0)
      {
         ...
         printf("t=%.2f,%.2f,%.2f,%.2f, a=%.2f,%.2f,%.2f,%.2f, ",
                t[0],t[1],t[2],t[3],a[0],a[1],a[2],a[3]);
         printf("np=%d,%d,%d,%d\n",np[0],np[1],np[2],np[3]);
         printf("delta = %.2e\n\n",d);
      }
   }
