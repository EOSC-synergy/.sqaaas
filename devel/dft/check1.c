...
static void check_dft4d_parms(void)
{
   ...
   if (bc_cstar()!=0)
      idp[1]=set_dft_parms(EXP,NPROC1*L1/2,1,0);
   else
      idp[1]=set_dft_parms(EXP,NPROC1*L1,0,0);
   if (bc_cstar()>1)
      idp[2]=set_dft_parms(EXP,NPROC2*L2,1,0);
   else
      idp[2]=set_dft_parms(EXP,NPROC2*L2,0,0);
   if (bc_cstar()>2)
      idp[3]=set_dft_parms(EXP,NPROC3*L3,1,0);
   else
...
{
   int my_rank,np,t,in,n,b,c,id,cs,bc;
