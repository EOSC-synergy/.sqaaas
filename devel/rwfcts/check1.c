...
* License (GPL)
*
* Analytic check of RWRAT reweighting factor.
*
*******************************************************************************/
...
static void read_rw_factors(void)
{
   ...
   error_root(rwp.rwfact!=RWRAT,1,"read_rw_factors [check1.c]",
              "This program works only for the RWRAT reweighting factor");
   ...
   error_root(np!=0,1,"read_rw_factors [check1.c]",
              "Invalid pole decomposition");
}
...
double calc_lnr(double sqn)
{
   ...
   f=pow(q2+rp.mu0*rp.mu0,(1.0*rp.power[0])/rp.power[1]);
   
   r=rp.A;
   for (k=0;k<rp.degree;k++)
   {
      r*=(q2+rp.nu[k]*rp.nu[k])/(q2+rp.mu[k]*rp.mu[k]);
   }
...
int main(int argc,char *argv[])
{
   ...
   if (my_rank==0)
   {
      ...
      printf("\n");
      printf("Analytic check of RWRAT reweighting factor\n");
      printf("------------------------------------------\n\n");
   ...
   rwp=rw_parms(0);
   rp=rat_parms(rwp.irp[0]);
   status=malloc(rwp.nfct*sizeof(int*));
   status[0]=malloc(3*rwp.nfct*sizeof(int));
   for (ifct=0;ifct<rwp.nfct;ifct++)
      status[ifct]=status[0]+3*ifct;
   ...
   for (isrc=0;isrc<rwp.nsrc;isrc++)
   {
      ...
      lnr1=rwrat(rwp.irp[0],rwp.nfct,rwp.np,rwp.isp,&sqn,status);
      lnr2=calc_lnr(sqn);
