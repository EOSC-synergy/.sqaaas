...
*
*   void sw_su3frc(double c)
*     Computes the SW part of the SU(3) quark force, using the current value
*     of the X tensor field (see the notes). The calculated force is then
*     multiplied by c and added to the MD SU(3) force field.
*
*   void hop_su3frc(double c)
*     Computes the hopping part of the SU(3) quark force, using the current
*     value of the X vector field (see the notes). The calculated force
*     is then multiplied by c and added to the MD SU(3) force field.
*
*   void sw_u1frc(double c)
*     Computes the SW part of the U(1) quark force, using the current value
*     of the X tensor field (see the notes). The calculated force is then
*     multiplied by c and added to the MD U(1) force field.
*
*   void hop_u1frc(double c)
*     Computes the hopping part of the U(1) quark force, using the current
*     value of the X vector field (see the notes). The calculated force
*     is then multiplied by c and added to the MD U(1) force field.
*
* Notes:
*
...
static complex_dble wd ALIGNED16;
...
void sw_su3frc(double c)
{
   ...
   error_root(query_flags(UDBUF_UP2DATE)!=1,1,
              "sw_su3frc [genfrc.c]",
...
void sw_u1frc(double c)
{
   ...
   bc=bc_type();
   dp=dirac_parms();
   c*=0.125*dp.u1csw*dp.qhat;

   mdfs=mdflds();
   fb=(*mdfs).u1frc;
   set_dvec2zero(7*(BNDRY/4),fb+4*VOLUME);
   ...
   for (n=0;n<6;n++)
   {
      ...
      for (ix=0;ix<VOLUME;ix++)
      {
         ...
         u1dagxu1(zb+ipu[2],zb+ipu[0],&wd);
         u1dagxu1(zb+ipu[3],&wd,&wd);
         cm1x1_retr(zb+ipu[1],&wd,&Z);
         
         Z*=c*(Xb[ipx[0]].c1+Xb[ipx[0]].c2+Xb[ipx[0]].c3
               +Xb[ipx[1]].c1+Xb[ipx[1]].c2+Xb[ipx[1]].c3
               +Xb[ipx[2]].c1+Xb[ipx[2]].c2+Xb[ipx[2]].c3
               +Xb[ipx[3]].c1+Xb[ipx[3]].c2+Xb[ipx[3]].c3);
