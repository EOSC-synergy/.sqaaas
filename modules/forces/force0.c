...
*
*   void plaq_su3frc(void)
*     Computes the force deriving from the SU(3) Wilson plaquette action,
...
* the programs plaq_su3frc() and force0() set the force field to zero.
*
...
void plaq_su3frc(void)
{
   ...
   error_root((gauge()&1)==0,1,
              "plaq_su3frc [force0.c]",
              "SU(3) gauge field is not active");
...
void force0(double c)
{
   ...
   else
   {
      ...
      error_root(query_flags(BSTAP_UP2DATE)!=1,1,
                 "force0 [force0.c]",
                 "Flags debug: Something is not working");
         ...
         for (n=0;n<3;n++)
         {
            ...
            if (c0!=1.0)
            {
               ...
               if ((t<(N0-1))||(bc==3))
               {
                  ...
                  _su3_alg_mul_add_assign(*(fdb+ip[1]),r1*bf,X);
               }

               if ((t>0)||(bc!=1))
               {
                  prod2su3alg(vd,wd+1,&X);
                  _su3_alg_mul_sub_assign(*(fdb+ip[2]),r1*bf,X);
               }

               su3dagxsu3(udb+ip[2],vd,wd+1);
               prod2su3alg(wd+1,wd,&X);
               _su3_alg_mul_sub_assign(*(fdb+ip[3]),r1*bf,X);
               ...
               su3xsu3dag(wd,udb+ip[2],wd+1);
               prod2su3alg(udb+ip[0],wd+1,&X);
               _su3_alg_mul_add_assign(*(fdb+ip[0]),r1*bf,X);
...
double action0(int icom)
{
   ...
   double r0,r1,trU[4],act,bf2,bf3;
   su3lat_parms_t lat;
   bc_parms_t bcp;
   ...
   for (ix=0;ix<VOLUME;ix++)
   {
      ...
      if ((t<(N0-1))||(bc!=0))
      {
         ...
         for (n=0;n<3;n++)
         {
            ...
            act+=(r0*trU[0]+c1*(bf2*trU[1]+trU[2]+bf3*trU[3]));
         }
      }
