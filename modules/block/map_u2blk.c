...
*     Assigns the global double-precision U(3) gauge field to the corresponding
...
static int bc,np[4],nmu[8],nbf[8],ofs[8];
...
void assign_ud2ubgr(blk_grid_t grid)
{
   ...
   if (NPROC>1)
   {
      ...
      error(iprms[0]!=(int)(grid),1,"assign_ud2ubgr [map_u2blk.c]",
   ...
   error((b==NULL)||((*b).u==NULL)||((*b).shf&0x4),1,
         "assign_ud2ubgr [map_u2blk.c]","Unallocated or improper block grid");
   ...
   set_grid_flags(grid,ASSIGNED_HD2HBGR);
}
