...
*     i labels the functions and x runs over the full range of the physical 
*     lattice (half the full lattice in the case of C* b.c.) coordinate in 
*     direction mu. While the range of x must be the same on all MPI processes, 
*     the number of functions stored may depend on the process id.
*
...
*                fixed coordinates rcpr[nu!=mu] (on the full machine, there
*                are thus RNPROC/nproc[mu] independent sets of functions).
*
*  nx[k]         Range of the coordinate x on the process with rcpr[mu]=k
*                and the same rcpr[nu!=mu] as the local process. nx need
...
*                rcpr[mu]=k. The number is m+1 if k<r and m if k>=r,
...
*                be nx[k] x {sum_j mf[j]} on the process with rcpr[mu]=k.
*
* The program dft_gather() moves the data among the processes at fixed
* rcpr[nu!=mu] such that the array f on the process with rcpr[mu]=0 contains
...
* In both cases
*
* RNPROC= nproc[0]*nproc[1]*nproc[2]*nproc[3];
* 
