...
int ipr_global(int *n)
{
   ...
   n1=n[1];
   n2=safe_mod(n[2],2*NPROC2);
   if (n2>=NPROC2)
   {
      n2-=NPROC2;
      if (bc_cstar()>=2) n1+=NPROC1/2;
   }
   n3=safe_mod(n[3],2*NPROC3);
   if (n3>=NPROC3)
   {
      n3-=NPROC3;
      if (bc_cstar()>=3) n1+=NPROC1/2;
   }
   n1=safe_mod(n1,NPROC1);
   
   NMIRR=1;
   im=0;
   if ((bc_cstar()!=0)&&(NBLK1%2==0))
   {
      NMIRR=2;
      im=n1/(NPROC1/NMIRR);
      n1=safe_mod(n1,NPROC1/2);
   }
   ...
   return im+NMIRR*(ip+NPROC_BLK*ib);
}
...
void ipt_global(int *x,int *ip,int *ix)
{
   ...
   x1=safe_mod(x[1],2*NPROC1*L1);
   x2=safe_mod(x[2],2*NPROC2*L2);
   x3=safe_mod(x[3],2*NPROC3*L3);
...
static void set_cpr(void)
{
   ...
   im=nr%NMIRR;
   ip=(nr/NMIRR)%NPROC_BLK;
   ib=nr/(NMIRR*NPROC_BLK);
   ...
   cpr[1]=(ib%(NBLK1/NMIRR))*NPROC1_BLK+(ip%NPROC1_BLK)+im*(NPROC1/2);
   ib/=(NBLK1/NMIRR);
   ip/=NPROC1_BLK;
...
void geometry(void)
{
   ...
   error((bc_cstar()!=0)&&(NPROC1%2!=0),1,"geometry [geometry.c]",
         "NPROC1 must be even with C* boundary conditions");
   error((bc_cstar()>=2)&&(NPROC2==1),1,"geometry [geometry.c]",
         "NPROC2 must be larger than 1 with C* boundary conditions in direction 2");
   error((bc_cstar()>=3)&&(NPROC3==1),1,"geometry [geometry.c]",
         "NPROC3 must be larger than 1 with C* boundary conditions in direction 3");
