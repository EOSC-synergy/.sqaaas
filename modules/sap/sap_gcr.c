...
* It assumes that the parameters of the Dirac operator and of the SAP
* preconditioner have been set through set_lat_parms(), set_dirac_parms1(),
* set_dirac_parms9() and set_sap_parms() (see the modules in the modules/flags
* directory).
*
               ...
               spinor_dble *eta,spinor_dble *psi,int *status)
{
   ...
   error_root((query_grid_flags(SAP_BLOCKS,HBGR_UP2DATE)!=1)||
              (query_flags(SWD_UP2DATE)!=1)||
              (query_grid_flags(SAP_BLOCKS,SW_UP2DATE)!=1),1,
              "sap_gcr [sap_gcr.c]",
              "Flags debug: Something is not working");

   if (spr.isolv==0)
   {
      error_root((query_grid_flags(SAP_BLOCKS,SW_E_INVERTED)==1)||
                 (query_grid_flags(SAP_BLOCKS,SW_O_INVERTED)==1),1,
                 "sap_gcr [sap_gcr.c]",
                 "Flags debug: Something is not working");
   }
   else if (spr.isolv==1)
