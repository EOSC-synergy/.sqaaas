...
*
* Based on openQCD-1.6/modules/udflds/bstap.c
* Copyright (C) 2012, 2013, 2016 Martin Luescher
*
* This software is distributed under the terms of the GNU General Public
* License (GPL)
*
* Allocation and calculation of the U(1) boundary staple field.
*
...
* conditions, the program set_u1_bstap() sets the time-like boundary staples
...
static int bc,np[4],nfc[8],ofs[8],hofs[8],tags[8],nmu[8];
static const complex_dble u1d0={0.0};
static complex_dble wd ALIGNED16;
static complex_dble *hdb=NULL;
...
static void alloc_hdb(void)
{
   ...
   error(iup[0][0]==0,1,"alloc_hdb [u1_bstap.c]",
   ...
   error(hdb==NULL,1,"alloc_hdb [u1_bstap.c]",
   ...
   for (ib=0;ib<(2*nfc[ifc]);ib++)
   {
      ...
      for (k=0;k<3;k++)
      {
         ...
         if (ifc&0x1)
         {
            ...
            {
               u1xu1dag(u1db+ip[3],u1db+ip[1],&wd);
               u1xu1(u1db+ip[2],&wd,sbuf);
            }
         }
         else
         {
            ...
            {            
               u1xu1(u1db+ip[0],u1db+ip[1],&wd);
               u1dagxu1(u1db+ip[2],&wd,sbuf);
            }
         }
...
static void send_staples(int ifc,int tag)
{
   ...
   nbf=12*nfc[ifc]; /* 18*3planes*2(sending and recieving in both directions). Here 18->2 */ 
