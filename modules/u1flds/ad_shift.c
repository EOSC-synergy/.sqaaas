...
*
* Based on openQCD-1.6/modules/udflds/shift.c
* Copyright (C) 2006, 2009, 2011-2013, 2016 Martin Luescher
*
* This software is distributed under the terms of the GNU General Public
* License (GPL)
*
* Translation of the global non-compact U(1) gauge field.
*
...
* automatically.
*
* The program shift_ad() acts globally and must be called simultaneously on
...
static double *sdbuf=NULL,*rdbuf;
...
static void alloc_idx(void)
{
   ...
   if (NPROC>1)
   {
      ...
      error(ib==NULL,1,"alloc_idx [ad_shift.c]",
   ...
   error((id==NULL)||(ib==NULL),1,"alloc_idx [ad_shift.c]",
         "Unable to allocate index arrays");
...
static void alloc_adbufs(void)
{
   ...
   sdbuf=amalloc(8*n*sizeof(double),ALIGN);
   error(sdbuf==NULL,1,"alloc_adbufs [ad_shift.c]",
...
static void get_adlinks(void)
{
   ...
   for (mu=0;mu<4;mu++)
   {
      if (np[mu]>1)
      {
         ...
         tag=mpi_tag();
         nbf=nlk[mu];        /*18 for SU(3) replaced with 1 for 'n-c' U(1).*/
         saddr=(*cl).saddr;
         raddr=(*cl).raddr;
         sb=sdbuf;
         rb=ab+(*cl).iu;
...
static void get_adstars(int ifc)
{
   int mu,*idx,*idm;
   int tag,ip,saddr,raddr,nbf;
   double *ab,*a,*sb,*rb;
   ...
   {
      tag=mpi_tag();
      nbf=4*npt[mu];  /*72 for SU(3) replaced with 4 for 'n-c' U(1).*/
      saddr=(*cs).saddr;
      raddr=(*cs).raddr;
      sb=sdbuf;
      rb=rdbuf;
...
static void shift_adstars(int ifc)
{
   int mu,t;
   int *id0,*id1,*idm;
   double *ab,*a;
...
int shift_ad(int *s)
{
   ...
   if (NPROC>1)
   {
      ...
      error((iprms[0]!=s[0])||(iprms[1]!=s[1])||
            ...
            "shift_ad [ad_shift.c]","Shift vector is not global");
   }
   ...
   mu=0;
   sr[mu]=safe_mod(s[mu],np[mu]*bs[mu]);
   if (sr[mu]>np[mu]*bs[mu]/2)
      sr[mu]-=np[mu]*bs[mu];
   ...
   for(mu=2;mu<4;mu++)
   {
      if (bc_cstar()>=mu)
      {
         sr[mu]=safe_mod(s[mu],2*np[mu]*bs[mu]);
         while(sr[mu]>np[mu]*bs[mu]/2)
         {
            sr[mu]-=np[mu]*bs[mu];
            sr[1]+=np[1]*bs[1]/2;
         }
   ...
   mu=1;
   sr[mu]=safe_mod(sr[mu],np[mu]*bs[mu]);
   if (sr[mu]>np[mu]*bs[mu]/2)
      sr[mu]-=np[mu]*bs[mu];
   ...
   error_root((sr[0]!=0)&&(bc_type()!=3),1,"shift_ad [ad_shift.c]",
